...
(define (find-constants-with-prefix pfx const-list)
  (let ((exe (try-compiling-defined-constants-tester
	      (map (lambda (x)
		     (list (symbol-with-prefix pfx (car x))
			   (cadr x)))
		   const-list))))
    (if (not exe) '()
	(dynamic-wind
...
(define socket-options-with-types
   '((debug "SO_DEBUG" boolean)
     (reuse-address "SO_REUSEADDR" boolean)
     (reuse-port "SO_REUSEPORT" boolean)
     (type "SO_TYPE" integer)
     (error "SO_ERROR" integer)
     (dont-route "SO_DONTROUTE" boolean)
     (broadcast "SO_BROADCAST" boolean)
     (send-buffer "SO_SNDBUF" integer)
     (receive-buffer "SO_RCVBUF" integer)
     (keep-alive "SO_KEEPALIVE" boolean)
     (oob-inline "SO_OOBINLINE" boolean)
     (no-check "SO_NO_CHECK" boolean)
     (priority "SO_PRIORITY" integer)
     (linger "SO_LINGER" linger)
     (pass-credentials "SO_PASSCRED" boolean)
     (peer-credentials "SO_PEERCRED" ucred)
     (receive-low-water "SO_RCVLOWAT" integer)
     (send-low-water "SO_SNDLOWAT" integer)
     (receive-timeout "SO_RCVTIMEO" timeout)
     (send-timeout "SO_SNDTIMEO" timeout)
     
     (security-authentication "SO_SECURITY_AUTHENTICATION" unused)
     (security-encryption-transport "SO_SECURITY_ENCRYPTION_TRANSPORT" unused)
     (security-encryption-network "SO_SECURITY_ENCRYPTION_NETWORK" unused)
     
     (bind-to-device "SO_BINDTODEVICE" string)
     
     (attach-filter "SO_ATTACH_FILTER" filter)
     (detach-filter "SO_DETACH_FILTER" filter)
     
     (peer-name "SO_PEERNAME" string)
     (timestamp "SO_TIMESTAMP" boolean)
     
     (accept-connect "SO_ACCEPTCONN" boolean)
     (pass-security-credentials "SO_PASSSEC" boolean)
     (peer-security-credentials "SO_PEERSEC" integer)
     (jumbo "SO_JUMBO" boolean)))
